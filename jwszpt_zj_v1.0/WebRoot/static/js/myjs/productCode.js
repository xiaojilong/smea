	...
	function saveD(){
		...
		var isListPageShow = $("#isListPageShow").val(); //是否列表显示
		var isDetailPageShow = $("#isDetailPageShow").val(); //是否详情显示
		...
		var fields = dname + ',xiaojl,' + dtype + ',xiaojl,' + dbz + ',xiaojl,' + isQian + ',xiaojl,' + isListPageShow + ',xiaojl,' + isDetailPageShow + ',xiaojl,' + ddefault ;
		//alert(fields);
		if(msgIndex == ''){
			arrayField(fields);
		}else{
	...
	function dialog_open(){
		...
		$("#isListPageShow").val('是');
		$("#form-field-radio6").attr("checked",true);
		$("#isDetailPageShow").val('是');
	...
	function editField(value,msgIndex){
		var efieldarray = value.split(',xiaojl,');
	...
	//赋值是否详情界面显示
	function isDetailPageShow(value){
		if(value == '是'){
			$("#isDetailPageShow").val('是');
		}else{
			$("#isDetailPageShow").val('否');
	...
	function appendC(value){
		//alert(value);
		var fieldarray = value.split(',xiaojl,');
		var fielBuffer = "";
		for(var i=0;i<fieldarray.length;i++){
			fielBuffer += '<td class="center">'+fieldarray[i]+'<input type="hidden" name="field0'+index+'" value="'+fieldarray[i]+'"></td>'
		}
	...
	function isSame(value){
		for(var i=0;i<arField.length;i++){
			var array0 = arField[i].split(',xiaojl,')[0];
			if(array0 == value){
				return false;
			}
		}

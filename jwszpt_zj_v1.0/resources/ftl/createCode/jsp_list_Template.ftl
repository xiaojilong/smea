  ...
  <div class="row-fluid">
							...
							<tr>
								...
								<td style="width: 30px;" class="center">
									...
									<div class='hidden-phone visible-desktop btn-group'>
										...
										<c:if test="${r"${QX.cha == 1 }"}">
										<button class='btn btn-mini btn-success' title="详情" onclick="detail('${r"${var."}${objectNameUpper}_ID${r"}"}');"><i class='icon-ok'></i></button>
										</c:if>
										<c:if test="${r"${QX.edit == 1 }"}">
										<button class='btn btn-mini btn-info' title="编辑" onclick="edit('${r"${var."}${objectNameUpper}_ID${r"}"}');"><i class='icon-edit'></i></button>
										</c:if>
										<c:if test="${r"${QX.del == 1 }"}">
										<button class='btn btn-mini btn-danger' title="删除" onclick="del('${r"${var."}${objectNameUpper}_ID${r"}"}');"><i class='icon-trash'></i></button>
										</c:if>
									</div>
								</td>
								<#list fieldList as var>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function detail(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="详情";
			 diag.URL = '<%=basePath%>${objectNameLower}/goDetail.do?${objectNameUpper}_ID='+Id;
			 diag.Width = 450;
			 diag.Height = 355;
			 diag.CancelEvent = function(){ //关闭事件
				nextPage(${r"${page.currentPage}"});

package org.xiaojl.controller.test.t4;
...
import org.xiaojl.service.test.t4.T4Service;
...
public class T4Controller extends BaseController {
	
	String menuUrl = "t4/list.do"; //菜单地址(权限用)
	@Resource(name="t4Service")
	...
	public Json save() throws Exception{
		...
		try {
			...
			pd.put("T4_ID", this.get32UUID());	//主键
	...
	public ModelAndView list(Page page){
		...
		try{
			pd = this.getPageData();
			page.setPd(pd);
			List<PageData>	varList = t4Service.list(page);	//列出T4列表
			mv.setViewName("test/t4/t4_list");
	...
	public ModelAndView goAdd(){
		logBefore(logger, "去新增T4页面");
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();
		try {
			mv.setViewName("test/t4/t4_edit");
	...
	public ModelAndView goEdit(){
		logBefore(logger, "去修改T4页面");
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();
		try {
			pd = t4Service.findById(pd);	//根据ID读取
			mv.setViewName("test/t4/t4_edit");
	...
	public ModelAndView goDetail(){
		logBefore(logger, "去查看T4详情页面");
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();
		try {
			pd = t4Service.findById(pd);	//根据ID读取
			mv.setViewName("test/t4/t4_detail");
			mv.addObject("msg", "edit");
			mv.addObject("pd", pd);
		} catch (Exception e) {
			logger.error(e.toString(), e);
	...
	public Json deleteAll() {
		...
		try {
			...
			if(null != DATA_IDS && !"".equals(DATA_IDS)){
				...
				t4Service.deleteAll(ArrayDATA_IDS);
			}
			pdList.add(pd);
			j.setSuccess(true);
			j.setMsg("操作成功!");
	...
	public ModelAndView exportExcel(){
		...
		try{
			...
			List<PageData> varOList = t4Service.listAll(pd);

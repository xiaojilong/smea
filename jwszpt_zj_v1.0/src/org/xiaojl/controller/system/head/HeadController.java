package org.xiaojl.controller.system.head;
...
import org.xiaojl.service.system.appuser.AppuserService;
import org.xiaojl.service.system.user.UserService;
import org.xiaojl.util.AppUtil;
import org.xiaojl.util.Const;
import org.xiaojl.util.PageData;
import org.xiaojl.util.SmsUtil;
import org.xiaojl.util.Tools;
import org.xiaojl.util.Watermark;
import org.xiaojl.util.mail.SimpleMailSender;
...
public class HeadController extends BaseController {
	...
	public Object sendEmail(){
		...
		if(null != strEMAIL && !"".equals(strEMAIL)){
			String strEM[] = strEMAIL.split(",xiaojl,");
			if(strEM.length == 4){
				if("yes".endsWith(isAll)){
					try {
						List<PageData> userList = new ArrayList<PageData>();
	...
	public ModelAndView goEditEmail() throws Exception{
		...
		if(null != strSMS1 && !"".equals(strSMS1)){
			String strS1[] = strSMS1.split(",xiaojl,");
			if(strS1.length == 2){
		...
		if(null != strSMS2 && !"".equals(strSMS2)){
			String strS2[] = strSMS2.split(",xiaojl,");
			if(strS2.length == 2){
		...
		if(null != strFWATERM && !"".equals(strFWATERM)){
			String strFW[] = strFWATERM.split(",xiaojl,");
			if(strFW.length == 5){
		...
		if(null != strIWATERM && !"".equals(strIWATERM)){
			String strIW[] = strIWATERM.split(",xiaojl,");
			if(strIW.length == 4){
	...
	public ModelAndView saveSys() throws Exception{
		...
		Tools.writeFile(Const.EMAIL,pd.getString("SMTP")+",xiaojl,"+pd.getString("PORT")+",xiaojl,"+pd.getString("EMAIL")+",xiaojl,"+pd.getString("PAW"));	//写入邮件服务器配置
		Tools.writeFile(Const.SMS1,pd.getString("SMSU1")+",xiaojl,"+pd.getString("SMSPAW1"));	//写入短信1配置
		Tools.writeFile(Const.SMS2,pd.getString("SMSU2")+",xiaojl,"+pd.getString("SMSPAW2"));	//写入短信2配置
		mv.addObject("msg","OK");
	...
	public ModelAndView saveSys2() throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		Tools.writeFile(Const.FWATERM,pd.getString("isCheck1")+",xiaojl,"+pd.getString("fcontent")+",xiaojl,"+pd.getString("fontSize")+",xiaojl,"+pd.getString("fontX")+",xiaojl,"+pd.getString("fontY"));	//文字水印配置
		Tools.writeFile(Const.IWATERM,pd.getString("isCheck2")+",xiaojl,"+pd.getString("imgUrl")+",xiaojl,"+pd.getString("imgX")+",xiaojl,"+pd.getString("imgY"));	//图片水印配置
		Watermark.fushValue();
		mv.addObject("msg","OK");
		mv.setViewName("save_result");
		return mv;
